...
# Equivocal Results for Predictive Models
...
# Finding a balance between improving performance and enough reportable results
test_pred <- augment(two_class_mod, testing_set)
test_pred %>% head()

library(probably)  # contains functions for equivocal zones

lvls <- levels(training_set$class)

test_pred <- 
  test_pred %>%
  # The buffer argument represents the bounds around 0.5 that make up the equivocal results
  mutate(.pred_with_eqz = make_two_class_pred(.pred_class_1, lvls, buffer = 0.15))

test_pred %>% count(.pred_with_eqz)

# Confusion matrices for all data and data with reportable results only (equivocal results removed)
# All data
...
# Determine how well the data fit on the test set
res_test <-
...
# Test the model on data a few years later
res_2020 <-
